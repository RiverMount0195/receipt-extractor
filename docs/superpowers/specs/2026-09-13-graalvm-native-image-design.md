...
2. **Runtime stage** — `FROM eclipse-temurin:25-jre` (see "Update 4" below for why this replaced `debian:bookworm-slim`). Installs `libtesseract-dev libleptonica-dev` via `apt-get` — the same packages the original JVM-based Dockerfile used, and the same ones CLAUDE.md documents for local dev; the `-dev` packages' compile-time symlinks (`libleptonica.so`, etc.) are what let Tess4J's/lept4j's JNA friendly-name loading resolve correctly, sidestepping the Debian Leptonica-naming quirk in "Update 3" below without a manual symlink. The bundled JRE in this image is unused (the app runs as a native executable, no JVM involved) — image size cost accepted in exchange for a runtime base already proven to work correctly with Tesseract/Leptonica. Copies the `tessdata` directory (same as today, since Tesseract's native layer still can't read a path inside a jar/executable) to `/app/tessdata` and sets `OCR_TESSDATA_PATH=/app/tessdata`. Copies the *entire* `build/native/nativeCompile/` directory from the build stage (executable and all sibling `.so` files together) to `/app/`, so they stay co-located exactly as GraalVM produced them. `ENTRYPOINT` runs the executable directly — no JVM, no `java -jar`, in the runtime image at all.
...
**Update 4:** after Update 3, a *fourth* error surfaced from the same real-Telegram-photo testing: `NoClassDefFoundError: Could not initialize class net.sourceforge.lept4j.Leptonica1` — the cached-failure form of the exact same Update 3 problem, this time reached via `Tesseract.setImage()` → `LeptUtils.convertImageToPix()`, a call every `doOCR()` makes (not just box/region iteration as originally thought). Once `Leptonica1`'s static initializer fails once in a running process, Java permanently marks the class erroneous, so every later reference throws this same `NoClassDefFoundError` regardless of call path — this was the same root cause as Update 3, just not yet confirmed fixed in the real container when it was reported (a rebuild from the Update 3 fix onward should have resolved it). Given four native-image/Debian-packaging gaps in a row, each caught only by chasing the *next* real-world failure one at a time, the runtime base was switched from `debian:bookworm-slim` to `eclipse-temurin:25-jre` with the `-dev` Tesseract/Leptonica packages — reverting to exactly the OS/package combination the original JVM-based Dockerfile used successfully, keeping only the native-image-specific parts (build stage, sibling-library copying, JPEG reflection metadata) that have nothing to do with which base image or package variant is used. This removes the manual Leptonica symlink from Update 3 (no longer needed — the `-dev` package provides it) and is expected to close out the whole class of Debian-packaging-naming issues, though it does not by itself address Updates 1-2 (sibling libraries, JPEG JNI metadata), which remain fixed and necessary regardless of base image.

**Update 5:** with real Telegram/Sheets credentials in production (past the base-image switch in Update 4), a *fifth* error surfaced — this one progress, not regression: `java.lang.IllegalArgumentException` / `NoSuchMethodException: com.google.api.services.sheets.v4.model.AppendValuesResponse.<init>()`, thrown from `SheetRowAppender.appendRow` while parsing the Sheets API's response. This is the exact residual risk the final whole-branch review flagged and explicitly accepted as a documented gap (see the review history around the original `@RegisterReflectionForBinding(ValueRange.class)` fix): with dummy credentials, every prior test run failed at OAuth token refresh before the request body was ever serialized, so the *response*-side reflection metadata for `AppendValuesResponse` (and its nested `UpdateValuesResponse`, which nests `ValueRange` again) was never exercised or verified — only real credentials succeeding all the way through surfaced it. Fixed by adding `AppendValuesResponse.class` and `UpdateValuesResponse.class` to the existing `@RegisterReflectionForBinding` on `SheetRowAppender`.

This one *was* verified with a full live round-trip, closing the gap the final review could only accept as documented risk: a local mock HTTP server stood in for both `oauth2.googleapis.com` (via `UserCredentials.Builder.setTokenServerUri`) and the Sheets API (via `Sheets.Builder.setRootUrl`), temporarily pointed at from `SheetsConfig` for this test only (reverted before committing). Rebuilding the native image and driving a real OCR request through it produced a real token refresh, a real `POST .../values/dummy:append` request carrying the serialized `ValueRange` body, a mock response shaped like a real Sheets API append response (including the nested `updates`/`updatedData` objects), and — with the fix in place — zero errors parsing it back. Without the fix, this same scenario reproduces the exact reported exception.

A real `docker build`/`run` and a real end-to-end Telegram photo message still haven't confirmed fixes 1-4 in the actual container/production path — those remain open per the "Not yet verified" list above. Update 5 is the first of these five fixes verified with a genuine live round-trip rather than local reproduction outside Docker or static metadata comparison.
