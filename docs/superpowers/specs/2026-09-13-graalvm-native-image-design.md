...
A real `docker build`/`run` and a real end-to-end Telegram photo message still haven't confirmed either fix in the actual container/production path — both remain open per the "Not yet verified" list above. Given two distinct native-image gaps have now surfaced from two different real-world exercises of this deployment, that verification is not just a formality — treat it as required, not optional, before the next production deploy.
