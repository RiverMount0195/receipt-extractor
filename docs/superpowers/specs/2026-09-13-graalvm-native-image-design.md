...
A real `docker build`/`run` and a real end-to-end Telegram photo message still haven't confirmed any of these four fixes in the actual container/production path — all remain open per the "Not yet verified" list above. Given four distinct native-image/Debian-packaging gaps have now surfaced from real-world exercise of this deployment, that verification is not just a formality — treat it as required, not optional, before the next production deploy.
