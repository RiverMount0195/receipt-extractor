...
- Requires a gitignored credentials file at `config/sheets-credentials.properties` (path configurable via `sheets.credentials-path` in `application.yml`), holding six keys: `client-id`, `client-secret`, `refresh-token`, `access-token`, `spreadsheet-id`, `sheet-name`. See `config/sheets-credentials.properties.example` for the required format, and `docs/superpowers/specs/2026-09-13-google-sheets-export-design.md` ("Obtaining the refresh token") for how to obtain these values via Google's OAuth Playground.
