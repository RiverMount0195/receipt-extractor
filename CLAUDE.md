...
- A GraalVM native-image build was tried (see `docs/superpowers/specs/2026-09-13-graalvm-native-image-design.md`) and reverted: three separate native-image-specific runtime crashes surfaced from real-world testing (missing sibling JDK libraries for AWT/ImageIO, missing JNI reflection metadata for the JPEG codec, and a Debian Leptonica package-naming mismatch) before a real Telegram photo message could be gotten to work end-to-end. The `org.graalvm.buildtools.native` Gradle plugin, `scripts/regenerate-native-image-config.sh`, and the checked-in reachability metadata are left in the repo unused, in case native-image is revisited later — they have no effect on this JVM-based Dockerfile or on `bootRun`/`test`.
