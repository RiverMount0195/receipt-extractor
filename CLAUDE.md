...
- On each write, the app looks up today's date (Asia/Ho_Chi_Minh, via an injected `Clock` bean) in column A and inserts a new row directly below that date row (shifting subsequent rows down), so repeated same-day entries stack newest-first immediately under the date. It always writes `Payment Method = "Chuyển khoản"`, `Income / Outcome = "Chi"`, `Description` = the OCR-derived/caption message, and `Amount` = the detected amount negated (blank if undetected). Bank source is not written to the sheet. If today's date isn't found in column A (wrong month, sheet not pre-populated, etc.), it falls back to appending the row at the end of the sheet instead.
