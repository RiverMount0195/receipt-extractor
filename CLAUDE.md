...
- After each successful OCR extraction, `SheetRowAppender.insertRow(amount, message)` writes a row to a configured Google Sheet (e.g. `"Tháng 12"`), using the user's personal Google OAuth2 credentials. This is best-effort — a Sheets failure never affects the OCR API's response.
- Sheet layout: column A holds pre-populated dates for the month as `d/M` text (e.g. `"1/12"` .. `"31/12"`, no leading zero); columns D–G are `Description, Payment Method, Income / Outcome, Amount` (columns B, C are unused by this app and left untouched). `sheets.sheet-name` is a fixed config value — update it manually each month (no automatic month rollover).
- On each write, the app looks up today's date (Asia/Ho_Chi_Minh, via an injected `Clock` bean) in column A. Each date row is expected to be followed by a blank template spacer row reserved for that day's entries: if the row directly below the date is empty, the app writes into it in place; if that row already holds an entry (from an earlier write that day), it inserts a new row directly below the existing entry instead (shifting subsequent rows down), so same-day entries stack chronologically under the date, using the spacer row first. If the date row has no row below it at all, it inserts a new row directly below the date row. It always writes `Payment Method = "Chuyển khoản"`, `Income / Outcome = "Chi"`, `Description` = the OCR-derived/caption message, and `Amount` = the detected amount negated (blank if undetected). Bank source is not written to the sheet. If today's date isn't found in column A (wrong month, sheet not pre-populated, etc.), it falls back to appending the row at the end of the sheet instead.
