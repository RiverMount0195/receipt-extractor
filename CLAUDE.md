...
- Sheet layout: column A holds pre-populated dates for the month as `d/M` text (e.g. `"1/12"` .. `"31/12"`, no leading zero); columns D–G are `Description, Payment Method, Income / Outcome, Amount` (columns B, C are unused by this app and left untouched).
